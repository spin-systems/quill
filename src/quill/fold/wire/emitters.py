...
class WireEmitter(BaseEmitter):
    ...
    def __init__(self, directory, name, verbose=False):
        ...
        if self._verbose:
            print(
                "Emit log: \n  " + "\n  ".join([str(x) for x in self.emit_log]),
                file=stderr,
            )
    ...
    def _index_emissions(self):
        lambda_sort = lambda x: [
            int(str(p)) if str(p).isnumeric() else str(p) for p in x.parts
        ]
        all_emission_dirs = sorted(
            set([p.parent for p in self.emit_log]), key=lambda_sort
        )
        if self._verbose:
            print(
                "Emit dirs:\n  " + "\n  ".join([str(x) for x in all_emission_dirs]),
                file=stderr,
            )
        rel_dirs = [d.relative_to(self.target_dir) for d in all_emission_dirs]
        if self._verbose:
            print(
                "All wire emitted dirs:\n  " + "\n  ".join([f"{d}" for d in rel_dirs])
            )
    ...
    def _index_emission_dir(self, e_dir, all_subdirs):
        ...
        if files_to_log:
            ...
            if len(e_dir.parts) == 0:  # i.e. the top-level path "."
                ...
                subdirs = [
                    d.relative_to(e_dir) for d in all_subdirs if d.parent == e_dir
                ]
